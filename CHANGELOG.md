...
- **标签系统**：每用户独立标签与彩色标记；设置页管理（新建、改色、合并、删除）
- **图片打标**：上传后单张/批量打标；图库按标签筛选（OR / AND）、批量打标与解标
- **操作日志扩展**：记录登录、删除、设置变更、标签操作等；支持日期、用户、操作类型、来源与 IP 筛选
- **API**：标签 CRUD、图片打标、`batch-tags`；上传支持 `tagIds`；列表/搜索/统计支持标签筛选

### 改进

- 操作日志页 UI 改版（操作/来源徽章、移除备注列）
- 设置侧栏增加「标签管理」

### 文档

- README 与文档站截图更新（上传、图库、标签、操作日志等）
